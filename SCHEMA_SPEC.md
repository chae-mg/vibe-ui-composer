...
Layout 컴포넌트의 편집 속성은 각 Node의 `props`에 저장한다. `Container`는 기본적으로 Column Stack이며, `Flex`는 Row / Column과 Wrap, Align, Justify, Gap, Padding, Height Mode를 지원한다. `Grid`는 Columns와 Gap, Align, Justify, Padding, Height Mode를 지원한다. Section과 Card는 Background, Text Color, Border, Radius, Shadow 속성을 지원한다. Puck 편집기와 독립 Renderer는 동일한 속성으로 스타일을 계산한다.
