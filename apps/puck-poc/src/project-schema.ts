...
export type ProjectNode = {
  ...
  responsive: Partial<Record<Exclude<Breakpoint, "desktop">, { gridSpan?: number }>>;
};
...
export function puckDataToProject(data: Data, name = "Puck PoC Project"): ProjectDocument {
...
export function projectToPuckData(project: ProjectDocument): Data {
  ...
  if (!root) {
    return { content: [], root: { props: { title: project.name } } };
  }
...
export function createDefaultProject(): ProjectDocument {
  ...
  const cardOne: ProjectNode = {
    ...
    layout: { gridSpan: 6 },
    responsive: { tablet: { gridSpan: 6 }, mobile: { gridSpan: 12 } }
  };
...
export function validateProjectRoundTrip(project: ProjectDocument) {
  ...
  const roundTrip = puckDataToProject(puckData, project.name);
  ...
  return {
    ...
    responsive: cardNode?.responsive.mobile?.gridSpan === 12,
